...
export class ProxyServer {
  ...
  constructor(upstreamUrl: string | URL, proxyPort: number, private logger?: TaggedLogger) {
      ...
      upgradeWebSocket((_c) => {
        ...
        return {
          onMessage: (msg, appClient) => {
            ...
            waitForCondition(() => upstream.readyState === WebSocketImpl.OPEN, 1000, 100)
          ...
          onOpen: (_evt, appClient) => {
            upstream.addEventListener("message", (msg) => {
              ...
              if (cached) {
                const ms = Math.round((performance.now() - cached.start) * 1000) / 1000;
                this.logger?.trace(
              ...
              waitForCondition(() => appClient.readyState === WebSocketImpl.OPEN, 1000, 100)
